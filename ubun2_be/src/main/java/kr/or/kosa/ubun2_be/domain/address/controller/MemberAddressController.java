...
public class MemberAddressController {
    ...
    public ResponseDto<?> updateAddress(@AuthenticationPrincipal CustomUserDetails customUserDetails, @PathVariable Long addressId, @Valid @RequestBody MemberAddressRegisterRequest addressRequest) {
    ...
    @Operation(summary = "회원 주소지 기본 주소로 설정")
    @PutMapping("/default/{addressId}")
    public ResponseDto<?> setDefaultAddress(@AuthenticationPrincipal CustomUserDetails customUserDetails, @PathVariable Long addressId) {
        addressService.setDefaultAddress(addressId, customUserDetails.getUserId());
        return ResponseDto.ok(null, "기본 주소로 설정했습니다.");
    }
