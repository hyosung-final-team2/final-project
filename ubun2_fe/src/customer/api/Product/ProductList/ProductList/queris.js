...
export const useRegisterProduct = (productRequest, imageFile, detailImageFiles, currentPage) => {
  const { sort, searchCategory, searchKeyword } = useProductTableStore();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: () => registerProduct(productRequest, imageFile, detailImageFiles),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['product', { page: currentPage, sort, searchCategory, searchKeyword }] }),
  });
};

export const useModifyProduct = (productRequest, imageFile, detailImageFiles, currentPage) => {
  const { sort, searchCategory, searchKeyword } = useProductTableStore();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: () => modifyProduct(productRequest, imageFile, detailImageFiles),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['product', { page: currentPage, sort, searchCategory, searchKeyword }] }),
  });
};
