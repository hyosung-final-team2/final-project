...
const MemberInsertModal = ({isOpen, setOpenModal, selectedMemberDetail, currentPage}) => {
    ...
    const { deletedPaymentMethods, deletedAddresses, setIsUpdateGlobal,addDeletedPaymentMethod,addDeletedAddresses, resetDeletedPaymentMethods,resetDeletedAddresses } = useMemberUpdateStore(state => ({
            ...
            setMemberData((prevState) => ({
                ...
                paymentMethods: prevState.paymentMethods.filter(pm =>
                    paymentMethod.paymentMethodType === 'CARD'
                        ? pm.cardNumber !== paymentMethod.paymentNumber
                        : pm.accountNumber !== paymentMethod.paymentNumber
                ),
            }));
        } else {
    ...
    const handleAddressDelete = (Address) => {
        if (!Address.addressId) {
            setMemberData((prevState) => ({
                ...prevState,
                addresses: prevState.addresses.filter(ad =>
                        ad.address !== Address.noneSplitAddress
                ),
            }));
        } else {
            addDeletedAddresses({
        ...
        >
            ...
            <Modal.Body>
                ...
                { !selectedMemberDetail.pending ?
                    <>
                        {/* 회원 결제수단 정보 */}
                        <PaymentInfoSection memberPaymentMethods={memberData?.paymentMethods} isUpdate={isUpdate} handlePaymentMethodAdd={handlePaymentMethodAdd} handlePaymentMethodDelete={handlePaymentMethodDelete}/>

                        {/* 회원 주소지 정보 */}
                        <AddressInfoSection memberAddresses={memberData?.addresses} isUpdate={isUpdate} handleAddressAdd={handleAddressAdd} handleAddressDelete={handleAddressDelete}/>
                    </>
                    :
                   null
