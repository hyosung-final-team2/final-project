...
import { formatBankAccount } from '../../../../utils/accountFormat.js';
import { maskCardNumber } from '../../../../utils/cardFormat.js';

const MemberPaymentTableRow = ({ paymentMethodId, number, paymentMethodType, paymentInstitution, paymentNumber, handleDelete }) => {
      ...
      <Table.Row style={{ position: 'relative' }}>
        ...
        <Table.Cell>
          {paymentMethodType === 'CARD' ? (
            <PaymentMethodBadge icon={CreditCardIcon} paymentType={paymentMethodType} />
          ) : (
            <PaymentMethodBadge icon={CurrencyDollarIcon} paymentType={paymentMethodType} />
          )}
        </Table.Cell>
        <Table.Cell>{paymentInstitution}</Table.Cell>
        <Table.Cell>
          {paymentMethodType === 'CARD' ? maskCardNumber(paymentNumber) : formatBankAccount(paymentInstitution.slice(0, -2), paymentNumber, true)}
        </Table.Cell>
