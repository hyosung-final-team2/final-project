  ...
  return (
    <div className='p-3'>
      <div className='mb-3 flex items-center justify-between'>
        ...
        {onlyInfo || <SearchableDropdown options={users} onSelect={handleSelectMember} />}
      </div>

      <div className='grid grid-cols-2 gap-6'>
        <InputLabel labelTitle='회원명' defaultValue={selectedMember.name} disabled={true} />
        <InputLabel labelTitle='이메일' defaultValue={selectedMember.email} disabled={true} />
        <InputLabel labelTitle='전화번호' defaultValue={selectedMember.phone} disabled={true} />
        <InputLabel labelTitle='가입일' defaultValue={selectedMember.createdAt} disabled={true} />
      </div>
    </div>
  );
};
