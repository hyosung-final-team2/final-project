...
const RegisterFirstStep = ({ setRegisterStep, setMemberName, setMemberEmail }) => {
  ...
  const buttonContainerClass = isKeyboardOn ? 'fixed bottom-0 left-0 w-full' : 'absolute bottom-6 left-6 right-6';

  const clickBtnStyle = isAllValuePossible ? 'bg-main text-white font-bold h-14' : 'bg-second text-main font-bold h-14';

  const buttonClass = isKeyboardOn ? `${clickBtnStyle} w-full rounded-none` : `${clickBtnStyle} w-full rounded-2xl`;
  ...
  return (
    <div className='relative max-w-[480px] mx-auto min-h-screen p-6 pb-20'>
      <div className='mb-6'>
        ...
        <InputText
          defaultValue={firstRegisterObj.memberName}
        ...
        />
        <InputTextWithBtn
          defaultValue={firstRegisterObj.memberEmail}
      ...
      <div className={buttonContainerClass}>
        <button onClick={() => submitForm()} className={buttonClass}>
          다음
        </button>
      </div>
    </div>
